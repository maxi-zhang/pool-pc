...
export default class EntrustToMeDevice extends React.Component{
    constructor(props){
        super(props);
    ...
    backCommon(){
        backCommon(ACTION.ENTRUST_TO_ME);
    }
    getEntrustToMeDevice(){
        let _this = this
        ...
        Axios.post('/device/device/list', qs.stringify(
            {
                ...
                'device_user_id':this.state[OPERATION.SERVER_INFO][ACTION.ENTRUST_TO_ME],
                'trust_user_id':this.state[OPERATION.USER_INFO][ACTION.ADMIN_USER_ID],
                'group_id': -1,
                'is_trusteeship': (this.state[OPERATION.SERVER_INFO][ACTION.ENTRUST_TO_ME])?1:5,
                'up_id': -1,
                'is_online': -1,
                'page': 1,
                'page_size': 10
